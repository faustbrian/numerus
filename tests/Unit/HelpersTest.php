...
describe('numerus() Helper Function', function (): void {
    ...
    describe('Edge Cases', function (): void {
        ...
        test('can use with mathematical operations', function (): void {
            expect(numerus(16)->sqrt()->value())->toBe(4.0);
            expect(numerus(2)->power(3)->value())->toBe(8);
            expect(numerus(10)->mod(3)->value())->toBe(1);
        });

        test('can use with percentage operations', function (): void {
            expect(numerus(25)->percentOf(100))->toBe(25.0);
            expect(numerus(100)->addPercent(20)->value())->toBe(120.0);
            expect(numerus(100)->subtractPercent(20)->value())->toBe(80.0);
        });

        test('can use with rounding operations', function (): void {
            expect(numerus(42.6)->round()->value())->toBe(43.0);
            expect(numerus(42.3)->ceil()->value())->toBe(43.0);
            expect(numerus(42.9)->floor()->value())->toBe(42.0);
            expect(numerus(-42)->abs()->value())->toBe(42);
        });

        test('can use with conversion methods', function (): void {
            expect(numerus(42.9)->toInt())->toBe(42);
            expect(numerus(42)->toFloat())->toBe(42.0);
        ...
        test('can create multiple independent instances', function (): void {
            ...
            expect($num3->value())->toBe(30);

            $result = $num1->plus($num2)->plus($num3);

            expect($result->value())->toBe(60);
            expect($num1->value())->toBe(10); // Original unchanged
        });
    });
});
