...
use Tests\Exceptions\SimulatedTestException;
        ...
        test('temporary locale restores after exception', function (): void {
            try {
                Numerus::withLocale('de', function (): void {
                    throw SimulatedTestException::create();
                });
            } catch (Exception) {
                // Expected
            }
...
describe('Static Parsing', function (): void {
    ...
    describe('Edge Cases', function (): void {
        ...
        test('formats very large numbers beyond PHP_INT_MAX', function (): void {
            $largeNumber = Numerus::create(\PHP_INT_MAX)->plus(\PHP_INT_MAX);
            $formatted = $largeNumber->format();

            expect($formatted)->toBeString();
            expect($formatted)->toContain(',');
        });

        test('abbreviates very large numbers beyond PHP_INT_MAX', function (): void {
            $largeNumber = Numerus::create(\PHP_INT_MAX)->plus(\PHP_INT_MAX);
            $abbreviated = $largeNumber->abbreviate();

            expect($abbreviated)->toBeString();
        });
    });
});
