 ...
 *
 * This class offers a functional approach to percentage calculations, making
 * it ideal for use in static contexts or when you don't need the full fluent
 * API provided by the Numerus class.
 *
 * ```php
...
final class Percentage
{
     ...
     *
     * @throws CannotCalculatePercentageException When total is zero (division by zero)
     *
     * @return float The percentage the part represents of the total (0-100 scale)
     */
    public static function of(int|float|Numerus $part, int|float|Numerus $total): float
    {
        ...
        throw_if($totalValue === 0 || $totalValue === 0.0, CannotCalculatePercentageException::zeroTotal());
     ...
     * ```
     *
     * @see absoluteDifferenceBetween() For unsigned difference magnitude
     ...
     * @throws CannotCalculatePercentageChangeException When original value is zero (division by zero)
    ...
    public static function differenceBetween(int|float|Numerus $original, int|float|Numerus $new): float
    {
        ...
        throw_if($originalValue === 0 || $originalValue === 0.0, CannotCalculatePercentageChangeException::fromZero());
     ...
     *
     * Note: The result is based on the first value as the baseline, so
     * absoluteDifferenceBetween(100, 80) and absoluteDifferenceBetween(80, 100)
     * will return different results (20% vs 25%).
     *
     * ```php
     ...
     * ```
     *
     * @see differenceBetween() For signed percentage difference
     * @param  float|int|Numerus                        $a The first value for comparison (used as baseline)
     * @param  float|int|Numerus                        $b The second value for comparison
     * @throws CannotCalculatePercentageChangeException When the first value is zero (division by zero)
     * @return float                                    The absolute percentage difference (always non-negative)
     */
     ...
     * ```
     *
     * @see add() To add a percentage to a number
     * @see subtract() To subtract a percentage from a number
     ...
     * markup calculations, price increases, tax additions, or compound adjustments.
     *
     * ```php
     ...
     * ```
     *
     * @see calculate() To get just the percentage amount
     ...
     * for discount calculations, price reductions, depreciation, or tax deductions.
     *
     * ```php
     ...
     *
     * @see calculate() To get just the percentage amount
     * @see add() For the inverse operation
