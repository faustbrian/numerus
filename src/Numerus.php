...
use Cline\Numerus\Facades\Math;
...
final readonly class Numerus implements Stringable
{
     ...
     *
     * @param float|int $value The numeric value to encapsulate. Can be an integer
     *                         or floating-point number. The value is stored internally
     *                         and cannot be modified after construction, ensuring
     *                         thread-safety and predictable behavior in calculations.
     */
    ...
    public static function average(array $values): self
    {
        ...
        $sum = array_reduce(
            $values,
            fn (int|float $carry, float|int|self $val): float|int => $carry + ($val instanceof self ? $val->value : $val),
            0,
        );
    ...
    {
        return Math::integerPart($this->value);
    }
    ...
    {
        return (float) Math::fractionalPart($this->value);
    }
    ...
    {
        $value = $addend instanceof self ? $addend->value : $addend;

        return new self($this->normalize(Math::add($this->value, $value)));
    }
    ...
    {
        $value = $subtrahend instanceof self ? $subtrahend->value : $subtrahend;

        return new self($this->normalize(Math::subtract($this->value, $value)));
    }
    ...
    {
        $value = $multiplier instanceof self ? $multiplier->value : $multiplier;

        return new self($this->normalize(Math::multiply($this->value, $value)));
    }
    ...
    {
        $value = $divisor instanceof self ? $divisor->value : $divisor;

        throw_if($value === 0 || $value === 0.0, InvalidArgumentException::class, 'Division by zero');

        return new self($this->normalize(Math::divide($this->value, $value)));
    }
    ...
    {
        return new self($this->normalize(Math::abs($this->value)));
    }
    ...
    {
        return new self($this->normalize(Math::ceil($this->value)));
    }
    ...
    {
        return new self($this->normalize(Math::floor($this->value)));
    }
    ...
    {
        return new self($this->normalize(Math::round($this->value, $precision, $mode)));
    }
    ...
    {
        $result = Math::round($this->value, $precision, RoundingMode::AwayFromZero);
    ...
    {
        $result = Math::round($this->value, $precision, RoundingMode::TowardsZero);
    ...
    {
        $result = Math::round($this->value, $precision, RoundingMode::PositiveInfinity);
    ...
    {
        $result = Math::round($this->value, $precision, RoundingMode::NegativeInfinity);
    ...
    {
        $result = Math::round($this->value, $precision, RoundingMode::HalfAwayFromZero);
    ...
    {
        $result = Math::round($this->value, $precision, RoundingMode::HalfTowardsZero);
    ...
    {
        $result = Math::round($this->value, $precision, RoundingMode::HalfEven);
    ...
    {
        $result = Math::round($this->value, $precision, RoundingMode::HalfOdd);
    ...
    {
        return new self($this->normalize(Math::negate($this->value)));
    }
    ...
    {
        return new self($this->normalize(Math::power($this->value, $exponent)));
    }
    ...
    {
        throw_if($this->value < 0, InvalidArgumentException::class, 'Cannot calculate square root of negative number');

        return new self($this->normalize(Math::sqrt($this->value)));
    }
    ...
    public function mod(int|float|self $divisor): self
    {
        ...
        throw_if($value === 0 || $value === 0.0, InvalidArgumentException::class, 'Modulo by zero');

        return new self($this->normalize(Math::mod($this->value, $value)));
    }
    ...
    {
        return is_int($this->value) || floor($this->value) === $this->value;
    }
    ...
    public function equals(int|float|self $other): bool
    {
        ...
        return Math::compare($this->value, $value) === 0;
    }
    ...
    {
        $minValue = $min instanceof self ? $min->value : $min;
        $maxValue = $max instanceof self ? $max->value : $max;
    ...
    public function min(int|float|self $other): self
    {
        ...
        return new self($this->normalize(Math::min($this->value, $value)));
    }
    ...
    public function max(int|float|self $other): self
    {
        ...
        return new self($this->normalize(Math::max($this->value, $value)));
    }
    ...
    public function clamp(int|float|self $min, int|float|self $max): self
    {
        ...
        throw_if($minValue > $maxValue, InvalidArgumentException::class, 'Min value cannot be greater than max value');

        return new self($this->normalize(Math::max($minValue, Math::min($maxValue, $this->value))));
    }
    ...
    public function percentOf(int|float|self $total): float
    {
        ...
        throw_if($totalValue === 0 || $totalValue === 0.0, InvalidArgumentException::class, 'Cannot calculate percentage of zero');
    ...
    {
        return new self($this->value + ($this->value * ($percent / 100)));
    }
    ...
    {
        $new = $newValue instanceof self ? $newValue->value : $newValue;

        throw_if($this->value === 0 || $this->value === 0.0, InvalidArgumentException::class, 'Cannot calculate percentage change from zero');

        return (($new - $this->value) / $this->value) * 100;
    }
    ...
    {
        $a = abs((int) $this->value);
        $b = abs($other instanceof self ? (int) $other->value : $other);
    ...
    {
        $otherValue = $other instanceof self ? (int) $other->value : $other;

        throw_if($this->value === 0 || $otherValue === 0, InvalidArgumentException::class, 'Cannot calculate LCM with zero');

        $gcd = $this->gcd($other)->value();

        return new self(abs(((int) $this->value * $otherValue) / $gcd));
    }
    ...
    {
        throw_if($this->value < 0, InvalidArgumentException::class, 'Cannot calculate factorial of negative number');
        throw_if(!is_int($this->value), InvalidArgumentException::class, 'Factorial requires an integer value');
    ...
    {
        /** @var string */
        return Number::abbreviate($this->value, $precision);
    }
    ...
    {
        /** @var string */
        return Number::forHumans($this->value, $precision);
    }
    ...
    {
        return Number::fileSize($this->value, $precision);
    }
    ...
    public function trim(): self
    {
        ...
        return new self(Number::trim($this->value));
    }

    private function normalize(int|float|string $value): int|float
    {
        if (is_string($value)) {
            return (float) $value;
        }
