...
use Cline\Numerus\Exceptions\CannotCalculateAverageException;
use Cline\Numerus\Exceptions\CannotCalculateFactorialException;
use Cline\Numerus\Exceptions\CannotCalculateLcmException;
use Cline\Numerus\Exceptions\CannotCalculatePercentageChangeException;
use Cline\Numerus\Exceptions\CannotCalculatePercentageException;
use Cline\Numerus\Exceptions\CannotCalculateSquareRootException;
use Cline\Numerus\Exceptions\CannotParseFloatException;
use Cline\Numerus\Exceptions\CannotParseIntegerException;
use Cline\Numerus\Exceptions\DivisionByZeroException;
use Cline\Numerus\Exceptions\FactorialRequiresIntegerException;
use Cline\Numerus\Exceptions\InvalidClampRangeException;
use Cline\Numerus\Exceptions\ModuloByZeroException;
 ...
 * @since 1.0.0
 */
final class Numerus implements JsonSerializable, Stringable
{
    use Macroable;
     ...
     *                                           for numeric inputs.
     * @throws CannotParseFloatException When a string value cannot be parsed as a valid
     ...
     * @throws CannotParseIntegerException When the string cannot be parsed as an integer
    ...
    public static function parseInt(string $value, ?string $locale = null): self
    {
        ...
        throw_if($result === false, CannotParseIntegerException::fromValue($value));
     ...
     * @param  null|string               $locale Optional locale identifier; uses default if not specified
     * @throws CannotParseFloatException When the string cannot be parsed as a float
    ...
    public static function parseFloat(string $value, ?string $locale = null): self
    {
        ...
        throw_if($result === false, CannotParseFloatException::fromValue($value));
     ...
     *
     * ```php
     * Numerus::sum([10, 20, 30]); // 60
     * Numerus::sum([numerus(5), 10, 15]); // 30
     * ```
     *
    ...
    /**
     * Calculate the sum of values resolved from each item via a selector.
     *
     * Resolves a numeric value from each item using either a dot-notated array
     * key or a callback, then sums all resolved values. Items where the key
     * is absent fall back to the default value.
     *
     * ```php
     * $orders = [['amount' => 100], ['amount' => 250], ['amount' => 75]];
     * Numerus::sumBy($orders, 'amount');                    // 425
     * Numerus::sumBy($orders, fn($o) => $o['amount'] * 2); // 850
     * ```
     *
     * @param array<mixed>                              $items   The collection of items to aggregate
     * @param callable(mixed, int|string): mixed|string $value   Dot-notated key or callback returning a numeric value per item
     * @param float|int|self                            $default Fallback value when the key is absent (default: 0)
     *
     * @return self A new Numerus instance containing the calculated sum
     */
    public static function sumBy(array $items, string|callable $value, int|float|self $default = 0): self
    {
        $sum = BigDecimal::zero();

        foreach ($items as $key => $item) {
            $resolved = self::resolveAggregateValue($item, $key, $value, $default);
            $sum = $sum->plus($resolved);
        }
    ...
    /**
     * Calculate the sum of pairwise products resolved from each item.
     *
     * For each item, resolves two values via separate selectors and multiplies
     * them together, then sums all products. Useful for weighted totals such
     * as (quantity × unit_price) across a collection of line items.
     *
     * ```php
     * $lines = [
     *     ['qty' => 2, 'price' => 10.0],
     *     ['qty' => 3, 'price' => 5.0],
     * ];
     * Numerus::sumProductBy($lines, 'qty', 'price'); // 35.0 (2*10 + 3*5)
     * ```
     *
     * @param array<mixed>                              $items        The collection of items to aggregate
     * @param callable(mixed, int|string): mixed|string $left         Dot-notated key or callback for the left factor per item
     * @param callable(mixed, int|string): mixed|string $right        Dot-notated key or callback for the right factor per item
     * @param float|int|self                            $leftDefault  Fallback when the left key is absent (default: 1)
     * @param float|int|self                            $rightDefault Fallback when the right key is absent (default: 0)
     *
     * @return self A new Numerus instance containing the sum of all products
     */
    public static function sumProductBy(
        array $items,
        string|callable $left,
        string|callable $right,
        int|float|self $leftDefault = 1,
        int|float|self $rightDefault = 0,
    ): self {
        $sum = BigDecimal::zero();

        foreach ($items as $key => $item) {
            $leftValue = self::resolveAggregateValue($item, $key, $left, $leftDefault);
            $rightValue = self::resolveAggregateValue($item, $key, $right, $rightDefault);

            $sum = $sum->plus($leftValue->multipliedBy($rightValue));
        }
    ...
    /**
     * Calculate the arithmetic mean of values resolved from each item via a selector.
     *
     * ```php
     * $scores = [['score' => 80], ['score' => 90], ['score' => 70]];
     * Numerus::averageBy($scores, 'score'); // 80.0
     * ```
     *
     * @param array<mixed>                              $items   The collection of items to aggregate
     * @param callable(mixed, int|string): mixed|string $value   Dot-notated key or callback returning a numeric value per item
     ...
     * @return self A new Numerus instance containing the calculated average
     */
    public static function averageBy(array $items, string|callable $value, int|float|self $default = 0): self
    {
        throw_if($items === [], CannotCalculateAverageException::emptyArray());

        $sum = BigDecimal::zero();

        foreach ($items as $key => $item) {
            $sum = $sum->plus(self::resolveAggregateValue($item, $key, $value, $default));
        }

        return new self($sum->dividedBy(count($items), 10, BrickRoundingMode::HALF_UP));
    }

    /**
     * Calculate the weighted average of values resolved from each item.
     *
     * Each item contributes its resolved value scaled by its resolved weight.
     * Useful when items have different levels of importance, such as grade
     * calculations where courses carry different credit weights.
     *
     * ```php
     * $courses = [
     *     ['credits' => 3, 'grade' => 90],
     *     ['credits' => 1, 'grade' => 70],
     * ];
     * Numerus::weightedAverageBy($courses, 'credits', 'grade'); // 85.0
     * ```
     *
     * @param array<mixed>                              $items         The collection of items to aggregate
     * @param callable(mixed, int|string): mixed|string $weight        Dot-notated key or callback returning the weight per item
     * @param callable(mixed, int|string): mixed|string $value         Dot-notated key or callback returning the value per item
     * @param float|int|self                            $weightDefault Fallback when the weight key is absent (default: 1)
     * @param float|int|self                            $valueDefault  Fallback when the value key is absent (default: 0)
     *
     * @throws InvalidArgumentException When the array is empty or total weight is zero
     *
     * @return self A new Numerus instance containing the weighted average
     */
    public static function weightedAverageBy(
        array $items,
        string|callable $weight,
        string|callable $value,
        int|float|self $weightDefault = 1,
        int|float|self $valueDefault = 0,
    ): self {
        throw_if($items === [], InvalidArgumentException::class, 'Cannot calculate weighted average of empty array');

        $weightedSum = BigDecimal::zero();
        $totalWeight = BigDecimal::zero();

        foreach ($items as $key => $item) {
            $weightValue = self::resolveAggregateValue($item, $key, $weight, $weightDefault);
            $resolvedValue = self::resolveAggregateValue($item, $key, $value, $valueDefault);

            $weightedSum = $weightedSum->plus($weightValue->multipliedBy($resolvedValue));
            $totalWeight = $totalWeight->plus($weightValue);
        }

        throw_if($totalWeight->isZero(), InvalidArgumentException::class, 'Cannot calculate weighted average with zero total weight');

        return new self($weightedSum->dividedBy($totalWeight, 10, BrickRoundingMode::HALF_UP));
    }

    /**
     * Find the minimum value resolved from each item via a selector.
     *
     * ```php
     * $products = [['price' => 10], ['price' => 5], ['price' => 20]];
     * Numerus::minBy($products, 'price'); // 5
     * ```
     *
     * @param array<mixed>                              $items   The collection of items to aggregate
     * @param callable(mixed, int|string): mixed|string $value   Dot-notated key or callback returning a numeric value per item
     ...
     *
     * @return self A new Numerus instance containing the minimum value
     */
    public static function minBy(array $items, string|callable $value, int|float|self $default = 0): self
    {
        throw_if($items === [], InvalidArgumentException::class, 'Cannot calculate minimum of empty array');

        $min = null;

        foreach ($items as $key => $item) {
            $resolved = self::resolveAggregateValue($item, $key, $value, $default);
            $min = !$min instanceof BigDecimal || $resolved->isLessThan($min) ? $resolved : $min;
        }
    ...
    /**
     * Find the maximum value resolved from each item via a selector.
     *
     * ```php
     * $products = [['price' => 10], ['price' => 5], ['price' => 20]];
     * Numerus::maxBy($products, 'price'); // 20
     * ```
     *
     * @param array<mixed>                              $items   The collection of items to aggregate
     * @param callable(mixed, int|string): mixed|string $value   Dot-notated key or callback returning a numeric value per item
     ...
     * @throws InvalidArgumentException When the array is empty
     *
     * @return self A new Numerus instance containing the maximum value
     */
    public static function maxBy(array $items, string|callable $value, int|float|self $default = 0): self
    {
        throw_if($items === [], InvalidArgumentException::class, 'Cannot calculate maximum of empty array');

        $max = null;

        foreach ($items as $key => $item) {
            $resolved = self::resolveAggregateValue($item, $key, $value, $default);
            $max = !$max instanceof BigDecimal || $resolved->isGreaterThan($max) ? $resolved : $max;
        }
    ...
    /**
     * Calculate the population variance of values resolved from each item.
     *
     * Measures the average squared deviation from the mean across the entire
     * population. Use sampleVarianceBy() when working with a sample of a
     * larger population and an unbiased estimate is needed.
     *
     * ```php
     * $data = [['v' => 2], ['v' => 4], ['v' => 4], ['v' => 6]];
     * Numerus::varianceBy($data, 'v'); // 2.0
     * ```
     *
     * @param array<mixed>                              $items   The collection of items to aggregate
     * @param callable(mixed, int|string): mixed|string $value   Dot-notated key or callback returning a numeric value per item
     ...
     * @throws InvalidArgumentException When the array is empty
     *
     * @return self A new Numerus instance containing the population variance
     */
    public static function varianceBy(array $items, string|callable $value, int|float|self $default = 0): self
    {
        throw_if($items === [], InvalidArgumentException::class, 'Cannot calculate variance of empty array');

        $values = self::resolveAggregateValues($items, $value, $default);
        $sum = BigDecimal::zero();
        ...
        $mean = $sum->dividedBy(count($values), 10, BrickRoundingMode::HALF_UP);
        $sumSquaredDiffs = BigDecimal::zero();

        foreach ($values as $resolved) {
            $sumSquaredDiffs = $sumSquaredDiffs->plus($resolved->minus($mean)->power(2));
        }

        return new self($sumSquaredDiffs->dividedBy(count($values), 10, BrickRoundingMode::HALF_UP));
    }

    /**
     * Calculate the population standard deviation of values resolved from each item.
     *
     * Returns the square root of the population variance, expressing spread in
     * the same unit as the original values. Use sampleStandardDeviationBy() when
     * working with a sample drawn from a larger population.
     *
     * ```php
     * $data = [['v' => 2], ['v' => 4], ['v' => 4], ['v' => 6]];
     * Numerus::standardDeviationBy($data, 'v'); // ~1.414
     * ```
     *
     * @param array<mixed>                              $items   The collection of items to aggregate
     * @param callable(mixed, int|string): mixed|string $value   Dot-notated key or callback returning a numeric value per item
     ...
     * @throws InvalidArgumentException When the array is empty
     *
     * @return self A new Numerus instance containing the population standard deviation
     */
    public static function standardDeviationBy(array $items, string|callable $value, int|float|self $default = 0): self
    {
        return self::varianceBy($items, $value, $default)->sqrt();
    }

    /**
     * Calculate the sample variance of values resolved from each item.
     *
     * Uses Bessel's correction (divides by n-1 instead of n) to provide an
     * unbiased estimate of the variance of the population from which the sample
     * was drawn. Requires at least two values to compute.
     *
     * ```php
     * $data = [['v' => 2], ['v' => 4], ['v' => 4], ['v' => 6]];
     * Numerus::sampleVarianceBy($data, 'v'); // ~2.667
     * ```
     *
     * @param array<mixed>                              $items   The collection of items to aggregate
     * @param callable(mixed, int|string): mixed|string $value   Dot-notated key or callback returning a numeric value per item
     * @param float|int|self                            $default Fallback value when the key is absent (default: 0)
     *
     * @throws InvalidArgumentException When the array is empty or contains fewer than two values
     *
     * @return self A new Numerus instance containing the sample variance
     */
    public static function sampleVarianceBy(array $items, string|callable $value, int|float|self $default = 0): self
    {
        throw_if($items === [], InvalidArgumentException::class, 'Cannot calculate sample variance of empty array');

        $values = self::resolveAggregateValues($items, $value, $default);

        throw_if(count($values) < 2, InvalidArgumentException::class, 'Cannot calculate sample variance with fewer than two values');
        ...
        return new self($sumSquaredDiffs->dividedBy(count($values) - 1, 10, BrickRoundingMode::HALF_UP));
    }

    /**
     * Calculate the sample standard deviation of values resolved from each item.
     *
     * Returns the square root of the sample variance, providing an unbiased
     * estimate of the population standard deviation from a sample. Requires
     * at least two values to compute.
     *
     * ```php
     * $data = [['v' => 2], ['v' => 4], ['v' => 4], ['v' => 6]];
     * Numerus::sampleStandardDeviationBy($data, 'v'); // ~1.633
     * ```
     *
     * @param array<mixed>                              $items   The collection of items to aggregate
     * @param callable(mixed, int|string): mixed|string $value   Dot-notated key or callback returning a numeric value per item
     ...
     * @throws InvalidArgumentException When the array is empty or contains fewer than two values
     *
     * @return self A new Numerus instance containing the sample standard deviation
     */
    public static function sampleStandardDeviationBy(
        array $items,
        string|callable $value,
        int|float|self $default = 0,
    ): self {
        return self::sampleVarianceBy($items, $value, $default)->sqrt();
    }

    /**
     * Calculate the median (50th percentile) of values resolved from each item.
     *
     * Sorts resolved values and returns the middle value, or the average of the
     * two middle values for even-length collections. Less sensitive to outliers
     * than the arithmetic mean.
     *
     * ```php
     * $data = [['v' => 1], ['v' => 3], ['v' => 2], ['v' => 4], ['v' => 5]];
     * Numerus::medianBy($data, 'v'); // 3
     * ```
     *
     * @param array<mixed>                              $items   The collection of items to aggregate
     * @param callable(mixed, int|string): mixed|string $value   Dot-notated key or callback returning a numeric value per item
     ...
     * @throws InvalidArgumentException When the array is empty
     *
     * @return self A new Numerus instance containing the median value
     */
    public static function medianBy(array $items, string|callable $value, int|float|self $default = 0): self
    {
        throw_if($items === [], InvalidArgumentException::class, 'Cannot calculate median of empty array');

        return self::percentileBy($items, 50, $value, $default);
    }

    /**
     * Calculate an arbitrary percentile of values resolved from each item.
     *
     * Supports two interpolation methods: 'linear' (default) interpolates between
     * adjacent values for non-integer rank positions; 'nearest_rank' returns the
     * value at the ceiling rank without interpolation. The 50th percentile is
     * equivalent to the median.
     *
     * ```php
     * $data = [['v' => 1], ['v' => 2], ['v' => 3], ['v' => 4], ['v' => 5]];
     * Numerus::percentileBy($data, 25, 'v');                     // 1.5 (linear)
     * Numerus::percentileBy($data, 75, 'v', 0, 'nearest_rank'); // 4
     * ```
     *
     * @param array<mixed>                              $items      The collection of items to aggregate
     ...
     * @param callable(mixed, int|string): mixed|string $value      Dot-notated key or callback returning a numeric value per item
     * @param float|int|self                            $default    Fallback value when the key is absent (default: 0)
     * @param string                                    $method     Interpolation method: 'linear' or 'nearest_rank' (default: 'linear')
     *
     * @throws InvalidArgumentException When the array is empty, the percentile is out of range, or the method is unknown
     *
     * @return self A new Numerus instance containing the calculated percentile value
     */
    public static function percentileBy(
        array $items,
        float $percentile,
        string|callable $value,
        int|float|self $default = 0,
        ...
        throw_if($items === [], InvalidArgumentException::class, 'Cannot calculate percentile of empty array');
        throw_if($percentile < 0 || $percentile > 100, InvalidArgumentException::class, 'Percentile must be between 0 and 100');
        throw_if(!in_array($method, ['linear', 'nearest_rank'], true), InvalidArgumentException::class, 'Unknown percentile method');

        $resolved = self::resolveAggregateValues($items, $value, $default);

        usort($resolved, static fn (BigDecimal $a, BigDecimal $b): int => $a->compareTo($b));

        if ($method === 'nearest_rank') {
            $rank = (int) ceil(($percentile / 100) * count($resolved));
            $index = max($rank - 1, 0);

            return new self($resolved[$index]);
        }

        $position = ($percentile / 100) * (count($resolved) - 1);
        $lowerIndex = (int) floor($position);
        $upperIndex = (int) ceil($position);

        if ($lowerIndex === $upperIndex) {
            return new self($resolved[$lowerIndex]);
        }

        $lower = $resolved[$lowerIndex];
        $upper = $resolved[$upperIndex];
        $fraction = BigDecimal::of($position - $lowerIndex);
        $interpolated = $lower->plus($upper->minus($lower)->multipliedBy($fraction));

        return new self($interpolated);
    }

    /**
     * Calculate the ratio of two summed selectors across all items.
     *
     * Sums the numerator selector across all items and divides by the sum of
     * the denominator selector. Useful for aggregate rate calculations such as
     * conversion rates, fill rates, or efficiency metrics.
     *
     * ```php
     * $campaigns = [
     *     ['clicks' => 50, 'impressions' => 1000],
     *     ['clicks' => 30, 'impressions' => 500],
     * ];
     * Numerus::ratioBy($campaigns, 'clicks', 'impressions'); // 0.0533... (CTR)
     * ```
     *
     * @param array<mixed>                              $items              The collection of items to aggregate
     * @param callable(mixed, int|string): mixed|string $numerator          Dot-notated key or callback for the numerator per item
     * @param callable(mixed, int|string): mixed|string $denominator        Dot-notated key or callback for the denominator per item
     * @param float|int|self                            $numeratorDefault   Fallback when the numerator key is absent (default: 0)
     * @param float|int|self                            $denominatorDefault Fallback when the denominator key is absent (default: 0)
     *
     * @throws DivisionByZeroException When the sum of denominator values is zero
     *
     * @return self A new Numerus instance containing the calculated ratio
     */
    public static function ratioBy(
        array $items,
        string|callable $numerator,
        string|callable $denominator,
        int|float|self $numeratorDefault = 0,
        int|float|self $denominatorDefault = 0,
    ): self {
        $numeratorSum = self::sumBy($items, $numerator, $numeratorDefault)->value;
        $denominatorSum = self::sumBy($items, $denominator, $denominatorDefault)->value;

        throw_if($denominatorSum->isZero(), DivisionByZeroException::create());

        return new self($numeratorSum->dividedBy($denominatorSum, 10, BrickRoundingMode::HALF_UP));
    }

    /**
     * Find the single most frequently occurring value resolved from each item.
     *
     * When multiple values share the highest frequency (a multimodal distribution),
     * returns the numerically smallest among them. Use modesBy() to retrieve all
     * mode values when ties are possible.
     *
     * ```php
     * $data = [['v' => 1], ['v' => 2], ['v' => 2], ['v' => 3]];
     * Numerus::modeBy($data, 'v'); // 2
     * ```
     *
     * @see modesBy() To retrieve all mode values when ties exist
     *
     * @param array<mixed>                              $items   The collection of items to aggregate
     * @param callable(mixed, int|string): mixed|string $value   Dot-notated key or callback returning a numeric value per item
     * @param float|int|self                            $default Fallback value when the key is absent (default: 0)
     ...
     * @throws InvalidArgumentException When the array is empty
     *
     * @return self A new Numerus instance containing the mode value
     */
    public static function modeBy(array $items, string|callable $value, int|float|self $default = 0): self
    {
        throw_if($items === [], InvalidArgumentException::class, 'Cannot calculate mode of empty array');

        $values = self::resolveAggregateValues($items, $value, $default);

        /** @var array<int, BigDecimal> $unique */
        $unique = [];
        ...
        foreach ($values as $resolved) {
            ...
            foreach ($unique as $index => $candidate) {
                if ($resolved->compareTo($candidate) === 0) {
                    $matchedIndex = $index;
        ...
        foreach ($counts as $index => $count) {
            ...
            if (!$unique[$index]->isLessThan($unique[$modeIndex])) {
                continue;
            }
        ...
        return new self($unique[$modeIndex]);
    }

    /**
     * Count the number of items that satisfy a predicate.
     *
     * ```php
     * $orders = [['status' => 'paid'], ['status' => 'pending'], ['status' => 'paid']];
     * Numerus::countWhere($orders, fn($o) => $o['status'] === 'paid'); // 2
     * ```
     *
     * @param array<mixed>                      $items     The collection of items to evaluate
     * @param callable(mixed, int|string): bool $predicate A callable that returns true for items to count
     *
     * @return int The number of items for which the predicate returned true
     */
    public static function countWhere(array $items, callable $predicate): int
    {
        $count = 0;
    ...
    /**
     * Count occurrences of each distinct value resolved from items.
     *
     * Groups items by their resolved value and returns an associative array
     * mapping each distinct value to its occurrence count. Useful for frequency
     * distributions and histogram data.
     *
     * ```php
     * $orders = [['status' => 'paid'], ['status' => 'pending'], ['status' => 'paid']];
     * Numerus::countBy($orders, 'status'); // ['paid' => 2, 'pending' => 1]
     * ```
     *
     * @param array<mixed>                              $items   The collection of items to aggregate
     * @param callable(mixed, int|string): mixed|string $value   Dot-notated key or callback returning the grouping value per item
     * @param float|int|self                            $default Fallback value when the key is absent (default: 0)
     *
     * @return array<int|string, int> Associative array mapping each distinct value to its count
     */
    public static function countBy(array $items, string|callable $value, int|float|self $default = 0): array
    {
        $values = self::resolveAggregateValues($items, $value, $default);
        $counts = [];
    ...
    /**
     * Find all mode values resolved from each item, sorted ascending.
     *
     * Unlike modeBy(), which returns only one value in a tie, this method
     * returns every value that shares the highest frequency. The result is
     * sorted in ascending numeric order.
     *
     * ```php
     * $data = [['v' => 1], ['v' => 2], ['v' => 2], ['v' => 3], ['v' => 3]];
     * Numerus::modesBy($data, 'v'); // [2, 3]
     * ```
     *
     * @see modeBy() To retrieve only the single smallest mode value
     *
     * @param array<mixed>                              $items   The collection of items to aggregate
     * @param callable(mixed, int|string): mixed|string $value   Dot-notated key or callback returning a numeric value per item
     * @param float|int|self                            $default Fallback value when the key is absent (default: 0)
     ...
     * @throws InvalidArgumentException When the array is empty
     *
     * @return array<float|int> All mode values sorted in ascending order
     */
    public static function modesBy(array $items, string|callable $value, int|float|self $default = 0): array
    {
        throw_if($items === [], InvalidArgumentException::class, 'Cannot calculate mode of empty array');

        $counts = self::countBy($items, $value, $default);
        $maxCount = 0;
        $modes = [];
    ...
    /**
     * Calculate the sum of squared values resolved from each item.
     *
     * Each resolved value is squared before summing. Useful as a building block
     * for statistical computations such as mean squared error, or for calculating
     * the magnitude of a vector.
     *
     * ```php
     * $data = [['v' => 3], ['v' => 4]];
     * Numerus::sumOfSquaresBy($data, 'v'); // 25 (3² + 4²)
     * ```
     *
     * @param array<mixed>                              $items   The collection of items to aggregate
     * @param callable(mixed, int|string): mixed|string $value   Dot-notated key or callback returning a numeric value per item
     * @param float|int|self                            $default Fallback value when the key is absent (default: 0)
     *
     * @return self A new Numerus instance containing the sum of squares
     */
    public static function sumOfSquaresBy(array $items, string|callable $value, int|float|self $default = 0): self
    {
        $sum = BigDecimal::zero();

        foreach (self::resolveAggregateValues($items, $value, $default) as $resolved) {
            $sum = $sum->plus($resolved->power(2));
        }
    ...
    /**
     * Serialize the numeric value for JSON encoding.
     *
     * Implements JsonSerializable interface to control JSON representation.
     * Returns the native numeric value (int or float) for optimal JSON output.
     *
     * @return float|int The numeric value for JSON serialization
     */
     ...
     *
     * @throws DivisionByZeroException When attempting to divide by zero
     *
    ...
    public function divideBy(int|float|self $divisor): self
    {
        ...
        throw_if($value->isZero(), DivisionByZeroException::create());
     ...
     *
     * @throws CannotCalculateSquareRootException When attempting to calculate square root of a negative number
     *
     * @return self A new Numerus instance with the square root value
     */
    public function sqrt(): self
    {
        throw_if($this->value->isNegative(), CannotCalculateSquareRootException::negativeNumber());
     ...
     *
     * @throws ModuloByZeroException When attempting modulo by zero
     *
    ...
    public function mod(int|float|self $divisor): self
    {
        ...
        throw_if($value->isZero(), ModuloByZeroException::create());
    ...
    /**
     * Create a negated assertion proxy for inverted comparison operations.
     *
     * Returns a proxy object that inverts all comparison methods, enabling
     * intuitive negated assertions without explicit negation operators.
     *
     * ```php
     ...
     * ```
     *
     * @return NegatedAssertion A proxy object with inverted comparison methods
     */
    public function not(): NegatedAssertion
    {
        return new NegatedAssertion($this);
    }
     ...
     * @throws InvalidClampRangeException When min is greater than max
    ...
    public function clamp(int|float|self $min, int|float|self $max): self
    {
        ...
        throw_if($minValue->compareTo($maxValue) > 0, InvalidClampRangeException::minGreaterThanMax());
     ...
     *
     * @throws CannotCalculatePercentageException When total is zero
     *
    ...
    public function percentOf(int|float|self $total): float
    {
        ...
        throw_if($totalValue->isZero(), CannotCalculatePercentageException::zeroTotal());
     ...
     *
     * @throws CannotCalculatePercentageChangeException When attempting to calculate from zero
     *
    ...
    public function percentageChange(int|float|self $newValue): float
    {
        ...
        throw_if($this->value->isZero(), CannotCalculatePercentageChangeException::fromZero());
     ...
     *
     * @throws CannotCalculateLcmException When either value is zero
     *
    ...
    public function lcm(int|self $other): self
    {
        ...
        throw_if($this->value->isZero() || $otherValue === 0, CannotCalculateLcmException::withZero());
     ...
     *
     * @throws CannotCalculateFactorialException|FactorialRequiresIntegerException When value is negative or not an integer
     *
     * @return self A new Numerus instance with the factorial value
     */
    public function factorial(): self
    {
        throw_if($this->value->isNegative(), CannotCalculateFactorialException::negativeNumber());
        throw_if(!$this->isInteger(), FactorialRequiresIntegerException::create());
    ...
    /**
     * Resolve an aggregate selector (array key or callback) to BigDecimal.
     *
     * @param callable(mixed, int|string): mixed|string $selector
     *
     * @throws InvalidArgumentException
     */
    private static function resolveAggregateValue(
        mixed $item,
        int|string $key,
        string|callable $selector,
        int|float|self $default,
    ): BigDecimal {
        if (is_string($selector)) {
            if (is_array($item) && array_key_exists($selector, $item)) {
                $resolved = $item[$selector] ?? $default;
            } elseif (is_array($item) && str_contains($selector, '.')) {
                $resolved = self::resolveAggregatePath($item, $selector) ?? $default;
            } else {
                $resolved = $default;
            }
        } else {
            $resolved = $selector($item, $key);
        }

        return self::normalizeAggregateValue($resolved);
    }

    /**
     * Resolve all aggregate values from an item array.
     *
     * @param  array<mixed>                              $items
     * @param  callable(mixed, int|string): mixed|string $selector
     * @return array<int, BigDecimal>
     */
    private static function resolveAggregateValues(array $items, string|callable $selector, int|float|self $default): array
    {
        $resolved = [];

        foreach ($items as $key => $item) {
            $resolved[] = self::resolveAggregateValue($item, $key, $selector, $default);
        }
    ...
    /**
     * Resolve a dot-notated path from an array item.
     *
     * @param array<mixed> $item
     */
    private static function resolveAggregatePath(array $item, string $path): mixed
    {
        $value = $item;
    ...
    /**
     * Normalize aggregate values into BigDecimal.
     *
     * @throws InvalidArgumentException
     */
    private static function normalizeAggregateValue(mixed $value): BigDecimal
    {
        ...
        throw new InvalidArgumentException('Resolver must return int, float, or Numerus');
    }
